...
function GalleryScene({
	...
	const spatialPositions = useMemo(() => {
		...
		for (let i = 0; i < visibleCount; i++) {
			// Alternate sides: 1 (right) or -1 (left)
			const side = i % 2 === 0 ? 1 : -1;
			const pairIndex = Math.floor(i / 2);
			
			// More organic horizontal distribution
			const x = side * (4 + (pairIndex % 4) * 0.8 + Math.sin(i * 2.3) * 0.5);
			
			// More organic vertical distribution
			const y = ((pairIndex % 6) - 2.5) * 1.8 + Math.cos(i * 1.7) * 0.4;
	...
	useFrame((state, delta) => {
		...
		if (scrollProgress !== undefined) {
			...
			// Calculate progress change since last frame
			const progressDelta = isFirstFrame.current ? 0 : currentProgress - prevProgress.current;
			
			prevProgress.current = currentProgress;
			isFirstFrame.current = false;
			
			// Map 0-1 progress to total depth movement
			// Multiply by a factor to control how "fast" the gallery moves relative to scroll
			currentScrollDelta = progressDelta * depthRange * 5 * scrollSpeed; 
			
			// Update velocity for shader effects (curving)
			setScrollVelocity(currentScrollDelta * 10);
		} else {
			// Apply auto-play
			if (autoPlay) {
			...
			// Damping for wheel/touch interaction
			setScrollVelocity((prev) => prev * 0.95);
			currentScrollDelta = scrollVelocity * delta * 10;
		}

		// Update time uniform for all materials - using clock's internal elapsed time if available
		...
		planesData.current.forEach((plane, i) => {
			let newZ = plane.z + currentScrollDelta;
			let wrapsForward = 0;
			let wrapsBackward = 0;
		...
		<>
			{planesData.current.map((plane, i) => {
				...
				const aspect = texture.image
					? (texture.image as { width: number; height: number }).width / (texture.image as { width: number; height: number }).height
					: 1;
		...
		<div className="flex flex-col items-center justify-center h-full bg-gray-100 p-4">
			...
			<div className="grid grid-cols-2 md:grid-cols-3 gap-4 max-h-96 overflow-y-auto">
				{normalizedImages.map((img, i) => (
					<div key={i} className="w-full h-32 relative">
						<Image
